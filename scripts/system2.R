...
generationAverageTemp <- 0

generationAverage <- numeric()

for(i in 1:52704){
  
  if(i%%6 != 0){
    
    generationAverageTemp <- generationAverageTemp + generation[i]
  }
  
  else{
    
    generationAverage[(i/6)] <- (generationAverageTemp + generation[i]) / 6
...
generationAverage <- rep(round(generationAverage), each=6)
...
for(i in 1:length(grid)){
  ...
  if((grid[i]+outputPowerHVAC1[i]-outputPowerHVAC2[i]) < -(lineCapacity*(1-lineSafetyMargin))){
    ...
    setPowerBESS[i] <- (generationAverage[i]-generation[i])
  }
...
ggplot(resultGrid, aes(x=time, y=before)) +
  ...
  geom_hline(yintercept=-182, size=0.1, color="red") +
  labs(x="time (month)", y="power (MW)") +
  theme(text=element_text(size=20)) +
  scale_x_datetime(date_labels="%m", date_breaks="1 month") +
  scale_y_continuous(limits=c(-190,130), breaks=seq(-200,140,20))

ggplot(resultGrid, aes(x=time, y=after)) +
  theme_bw() +
  geom_line(size=0.1, color="#56B1F7") +
  geom_hline(yintercept=-182, size=1, color="red") +
  labs(x="time (month)", y="power (MW)") +
  theme(text=element_text(size=20)) +
  scale_x_datetime(date_labels="%m", date_breaks="1 month") +
  scale_y_continuous(limits=c(-190,130), breaks=seq(-200,140,20))
